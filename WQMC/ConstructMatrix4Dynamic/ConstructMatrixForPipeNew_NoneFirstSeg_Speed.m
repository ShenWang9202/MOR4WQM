function A_Pipe = ConstructMatrixForPipeNew_NoneFirstSeg_Speed(delta_t,CurrentFlow,EnergyMatrixPipe,ElementCount,IndexInVar,aux,PipeReactionCoeff_perSec)
...
CurrentFlowPipe = CurrentFlowPipe/ConfigurationConstants.GPMperCFS;
LinkDiameterPipe = LinkDiameterPipe'/ConfigurationConstants.FT2Inch;
% don't use the one provided by EPANET, we need the direction of Velocity in pipes
CurrentVelocityPipe = CurrentFlowPipe./(ConfigurationConstants.pi*(0.5.*LinkDiameterPipe).^2);
...
NoneZero = (sum(NumberofSegment4Pipes)-PipeCount)*3;
iVector = zeros(1,NoneZero);
jVector = zeros(1,NoneZero);
vVector = zeros(1,NoneZero);
tempI = 1;
tempJ = 1;
tempV = 1;
for i = 1:PipeCount
    ...
    for seg = secondSegment:lastbutOneSegment
        iVector(tempI:tempI+2) = seg;
        tempI = tempI + 3;
        jVector(tempJ:tempJ+2) = seg - 1:seg + 1;
        tempJ = tempJ + 3;
        vVector(tempV:tempV+2) = vVect;
        tempV = tempV + 3;
    end
