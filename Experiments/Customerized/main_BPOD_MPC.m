function main_BPOD_MPC(NO_Plots,saveFileName)
...
switch Network
    ...
    case {5,6,7}
        ...
        flowRate_B = 10*ones(size(Location_B)); % unit: GPM
        Price_B = 0.01*ones(size(Location_B));
        Location_S = {'J10','J22','J23'};
        TargetedPipeID = [];%PipeID([2,4,6,8]);
        POD_Steps = 8000;
        DurationofResponseTested = 60000; %in seconds
        EnergyPercentageGoal_BT = 99.999999999995/100;
        EnergyPercentageGoal_BPOD = 99.9999999995/100;
        EnergyPercentageGoal_POD = 99.999999/100;
    case 8
        Location_B = {'J2'}; % NodeID here;
    ...
    case 9
        ...
        Location_S = {'J237','J247','J255','J241','J249'};
        flowRate_B = 10*ones(size(Location_B)); % unit: GPM
        Price_B = ones(size(Location_B));
...
%             profile on
            if(~NO_Plots)
                [sysFull,ReducedsystemMatrix_BT,ReducedsystemMatrix_POD,ReducedsystemMatrix_BPOD,timeCost] = ModelReduction(FullSystemModel,ReductionParameter,tInMin);
            else
                [sysFull,ReducedsystemMatrix_BT,ReducedsystemMatrix_POD,ReducedsystemMatrix_BPOD,timeCost] = ModelReduction_Noplots(FullSystemModel,ReductionParameter,tInMin);
            end
...
save(filename,'NetworkName',...
    'Location_S','Location_B','NumberofSegment4Pipes',...
    'Variable_Symbol_Table2','YY_estimated',...
    'X_Junction_control_result','NodeID',...
    'ControlActionU')
end
