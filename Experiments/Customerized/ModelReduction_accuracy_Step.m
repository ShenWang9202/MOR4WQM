function [sysFull,ReducedsystemMatrix_BT,ReducedsystemMatrix_POD,ReducedsystemMatrix_BPOD,timeCost]= ModelReduction_accuracy_Step(PreviousSystemDynamicMatrix,ReductionParameter,tInMin)
%% initialize
sysBT = [];
...
[Yfull,~] = step(50*sys,TimeLast);%[~,Yfull] = impluseDiscrete(sys.A,sys.B,sys.C,sys.D,sys.ts,steps,outformat);
tRes = toc;
...
if ConfigurationConstants.BT
    ...
    tic
    [YBT,~] = step(50*sysBT,TimeLast);%[~,YBT] = impluseDiscrete(sysBT.A,sysBT.B,sysBT.C,sysBT.D,sysBT.ts,steps,outformat);
    tRes = toc;
...
if ConfigurationConstants.POD
    ...
    tic
    [YPOD,~] = step(50*sysPOD,TimeLast);%[~,YPOD] = impluseDiscrete(sysPOD.A,sysPOD.B,sysPOD.C,sysPOD.D,sysPOD.ts,steps,outformat);
    tRes = toc;
...
if ConfigurationConstants.BPOD
    ...
    tic
    [YBPOD,~] = step(50*sysBPOD,TimeLast);%[~,YBPOD] = impluseDiscrete(sysBPOD.A,sysBPOD.B,sysBPOD.C,sysBPOD.D,sysBPOD.ts,steps,outformat);
    tRes = toc;
...
dimension = struct('full',nx,...
    ...
    'rPOD',rPOD,...
    'rBPOD',rBPOD);
% plot impulse response accuracy
% plotAccuracy4paper_Net1(TimeLast,YData,dimension);
% plotAccuracy4paper_Net3(XData,YData,dimension);

% plot step response accuracy
plotAccuracy4paper_Net1_Step(TimeLast,YData,dimension);

fig2 = figure;
for i = 1:Counter
    subplot(nc,nu,i)
    stairs(TimeLast,YData{i});
end
