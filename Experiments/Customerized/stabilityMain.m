function [Yfull,YBPOD,delta_t] = stabilityMain(PODStep_Settings,fullResponse,stablization)
...
switch Network
    ...
    case {5,6,7}
        ...
        Location_S = {'J22','J23'};
        TargetedPipeID = [];%PipeID([2,4,6,8]);
        POD_Steps = PODStep_Settings;
        DurationofResponseTested = 80000; %in seconds
        EnergyPercentageGoal_BT = 99.999999999995/100;
        EnergyPercentageGoal_BPOD = 99.999999/100;
        EnergyPercentageGoal_POD = 99.99999999999/100;
    otherwise
        disp('other value')
end
...
% profile on
tic
while (tleft > 0 && tInMin < Hq_min - 1 && delta_t <= 60)
...
%             end
            ...
            [Yfull,YBPOD] = ModelReduction_stability(FullSystemModel,ReductionParameter,tInMin);
