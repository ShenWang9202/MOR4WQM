...
%         EnergyPercentageGoal_BT = 99.93/100;
%         EnergyPercentageGoal_BPOD = 99.94/100;
...
while (tleft > 0 && tInMin < SimutionTimeInMinute && delta_t <= 60)
    ...
    if(mod(tInMin,Hq_min) == 0)
        ...
        if(MODELREDUCTION)
            ...
            % Using impluse response to verify the effectiveness of reduced
            % order 
            %[sysFull,ReducedsystemMatrix_BT,ReducedsystemMatrix_POD,ReducedsystemMatrix_BPOD,timeCost] = ModelReduction_e1(FullSystemModel,ReductionParameter,tInMin);
            
            % Using step response to verify the effectiveness of reduced
            % order 
            [sysFull,ReducedsystemMatrix_BT,ReducedsystemMatrix_POD,ReducedsystemMatrix_BPOD,timeCost] = ModelReduction_e1_step(FullSystemModel,ReductionParameter,tInMin);
